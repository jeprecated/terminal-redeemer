...
expected_version=${EXPECTED_ZELLIJ_VERSION:-0.43.1}
...
mkdir -m 700 -p "$real_base/$version" "$real_cache" "$shim_cache" "$private_root"
...
cleanup() {
  if [[ -d $real_base/$version ]]; then
    for socket in "$real_base/$version"/*; do
      [[ -S $socket ]] || continue
...
make_private_exact_socket() {
  ...
  local dir=$private_root/$attachment/$version
  mkdir -p "$dir"
  chmod 700 "$private_root/$attachment" "$dir"
  ln "$real_base/$version/$session" "$dir/$session"
...
real_inode=$(stat -c '%d:%i' "$real_base/$version/exact-session")
private_inode=$(stat -c '%d:%i' "$exact_private/$version/exact-session")
...
mkdir -m 700 -p "$private_root/att-symlink/$version"
ln -s "$real_base/$version/exact-session" "$private_root/att-symlink/$version/exact-session"
set +e
...
mkdir -p "$real_cache/zellij/$version/session_info/$dead_session"
printf 'layout {\n    pane\n}\n' >"$real_cache/zellij/$version/session_info/$dead_session/session-layout.kdl"
...
mkdir -m 700 -p "$private_root/att-dead/$version"
...
[[ ! -S $real_base/$version/$dead_session ]] || fail "isolated attach resurrected the dead session"
expect_attached "$(attach_command "$real_base" "$real_cache" "$dead_session" "$host_config")"
[[ -S $real_base/$version/$dead_session ]] || fail "normal-cache control did not resurrect the dead session"
...
for _ in $(seq 1 50); do
  [[ ! -S $real_base/$version/prefix ]] && break
  sleep 0.05
done
...
for _ in $(seq 1 50); do
  [[ ! -S $real_base/$version/quit-control ]] && break
  sleep 0.05
done
[[ ! -S $real_base/$version/quit-control ]] || fail "hostile quit control did not terminate its test session"
...
exact_private_path=$exact_private/$version/exact-session
...
long_candidate=$private_root/att-long/$version/$long_session
...
mkdir -p "$private_root/keep" "$private_root/att-stale/$version"
ln "$real_base/$version/exact-session" "$private_root/att-stale/$version/exact-session"
